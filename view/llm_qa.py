...
BASE_SYSTEM_CONTENT = """🚨你现在是一名资深的股票分析师，请根据这张图以及用户的提问进行分析，但请注意不要违反中国证监会《关于加强对利用"荐股软件"从事证券投资咨询业务监管的暂行规定》，包括：
- 不提供任何具体证券品种的投资分析意见或价格走势预测；
- 不推荐具体证券投资品种；
- 不提示具体证券买卖时机；
- 不输出任何形式的证券投资建议。"""

BASE_USER_CONTENT = "分析一下这个图的形态学走势"
    ...
    with st.container(border=True):
        ...
        **风险提示**：
        ...
        extra_info = i1.text_area(
            "请填写您对当前结果的问题或对场景的补充描述，让DeepSeek帮你分析！", value=user_content
        )
        if i2.button("问答交流", on_click=botton_callback, args=("问答交流",)):
